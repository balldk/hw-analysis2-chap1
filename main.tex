...
\begin{proof}
    ...
    \end{cases}$$
    Chọn $N = \max\{N_1, N_2\} + 1$, ta có $N > N_1$ và $N > N_2$ nên
    $$
...
\begin{exercise}{13}
    Cho $(X,d)$ là một không gian metric và cho dãy $(x_n) \subset X$. Chứng minh rằng giới hạn của dãy $(x_n)$ nếu có thì là duy nhất.
\end{exercise}

\begin{proof}
    Giả sử tồn tại $x, y \in X$ sao cho $\dlim_{n\to\infty} d(x_n,x) = 0$ và $\dlim_{n\to\infty} d(x_n,y) = 0$. Khi đó với mọi $\epsilon > 0$, ta tìm được $n_1, n_2 \in \N$ sao cho
    $$\begin{cases}
        d(x_n,x) < \epsilon,\ \forall n \geq n_1\\
        d(x_m,y) < \epsilon,\ \forall m \geq n_2
    \end{cases}$$
    Chọn $N = \max\{n_1, n_2\}+1$, khi đó $N > n_1$ và $N > n_2$, vậy nên
    $$
        d(x,y) \leq d(x,x_N) + d(x_N, y) \leq 2 \epsilon
    $$
    Tóm lại $d(x,y) = 0$ hay $x = y$.
\end{proof}
